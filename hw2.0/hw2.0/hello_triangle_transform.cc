...
void tri()
{
    ...
    {
        vec3 n1 = normalize(cross(ctm*vertices[1+3*i] - ctm*vertices[0+3*i],
                                  ctm*vertices[2+3*i] - ctm*vertices[1+3*i]));
        ...
        // flat shading, we will ingore the specular component for now.
            points[0+3*i] = ctm*vertices[0+3*i];
            colors[0+3*i] = ambient_color + diffuse_color;
            
            points[1+3*i] = ctm*vertices[1+3*i];
            colors[1+3*i] = ambient_color + diffuse_color;
            
            points[2+3*i] = ctm*vertices[2+3*i];
            colors[2+3*i] = ambient_color + diffuse_color;
    }
    std::cout <<"points first element "<< points[85727].x <<" y "<<points[85727].y<<" z "<<points[85727].z<<std::endl;
    std::cout <<"colors first element "<< colors[85727].x <<" y "<<colors[85727].y<<" z "<<colors[85727].z<<std::endl;
}
...
void init()
{
    ...
    std::cout<<"The size "<<sizeof(points[0])*NumVertices+sizeof(colors[0])*NumVertices<<std::endl;
    glBufferData(GL_ARRAY_BUFFER, sizeof(points[0])*NumVertices+sizeof(colors[0])*NumVertices, NULL, GL_STATIC_DRAW);
    ...
    glVertexAttribPointer(loc2, 4, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(sizeof(points[0])*NumVertices));
...
void display( void )
{
    ...
    glBufferSubData( GL_ARRAY_BUFFER, sizeof(points[0])*NumVertices,sizeof(colors[0])*NumVertices, colors);
...
int main(int argc, char** argv)
{
    ...
    parser.read_wavefront_file(argv[1], tri_ids, tri_verts);
    vertices = new point4[tri_ids.size()];
    // build all the surfaces:
    for (int k = 0; k < (int)tri_ids.size() / 3; ++k) {
       vertices[3*k] = point4(tri_verts[3*tri_ids[3*k]],
                  tri_verts[3*tri_ids[3*k]+1],
                  tri_verts[3*tri_ids[3*k]+2],
                  1.0);
        
       vertices[1+3*k] = point4(tri_verts[3*tri_ids[3*k+1]],
                  tri_verts[3*tri_ids[3*k+1]+1],
                  tri_verts[3*tri_ids[3*k+1]+2],
                  1.0);
        
        vertices[2+3*k] = point4(tri_verts[3*tri_ids[3*k+2]],
                  tri_verts[3*tri_ids[3*k+2]+1],
                  tri_verts[3*tri_ids[3*k+2]+2],
                  1.0);
    }
        //std::cout << "found this many triangles: " << vertices.size ()<< std::endl;
        //std::cout <<" first element "<< vertices[0].x <<" y "<<vertices[0].y<<" z "<<vertices[0].z<<std::endl;
